...
\begin{itemize}
    ...
    \item Function to generate hashes and random numbers. We needed this for the proof of work algorithm, which required randomized \verb|nonce| values, and miners to calculate valid hashes for each block. For this, we made use of \textbf{Libsodium}, a library which provides cryptography related functionality.
...
\begin{figure}[htp]
    \centering
    \subfigure[Centralised]{\includegraphics[height=45mm, rotate=90, origin=c]{centralised.png}}
    \qquad
    \subfigure[Decentralised]{\includegraphics[height=45mm, rotate=90, origin=c]{decentralised.png}}
...
A critical problem we had to solve was to develop a concurrent solution to each node's mining process. If a node is in the midst of mining and a new blockchain is propagated to it, the node needs to update its own blockchain before continuing with its mining. Our solution to solving this is through the combination of mutexes and our asynchronous network, which listens for blockchain updates on listener threads while the main thread does the mining. This mutex was reused when we implemented asynchronous mempool appending when nodes conduct transactions.

Another issue we faced was the combination of \verb|Binn| and \verb|NNG|. \verb|NNG| was used to set up our networks and send data from one connected node to another, while \verb|Binn| was used as our payloads to be sent. However, memory allocation for these two are complicated and become a problem quickly. 
